...
from utils.strive_dataset_processing.strive_dataset_reader import StriveDatasetReader
...
from utils.interaction_dataset.scenario_track_info import ScenarioTrackInfo
from utils.interaction_dataset.agent_track_info import AgentTrackInfo


class StriveDatasetScenarioGeneration(ScenarioGeneration):
    """This class generates a list of BARK scenarios from (adversarial) scenarios generated using the STRIVE model."""
    def __init__(self, params=None, num_scenarios=None, random_seed=None):
     super().__init__(params, num_scenarios, random_seed)

    def initialize_params(self, params):
        """This function initializes the parameters of the scenario generation by processing STRIVE scenario file. 
        The parameters are extracted from the json file under ["Scenario"]["Generation"]["StriveDatasetScenarioGeneration"].
        Attention: This scenario generation can only generate one scenario at a time as mentioned here: reply mode.
        ...
        params_temp = \
            self._params["Scenario"]["Generation"]["StriveDatasetScenarioGeneration"]
        self._map_file_name = params_temp["MapFilename",
                                          "Path to the open drive map",
                                          os.path.expanduser('~') + "/STRIVE/data/interaction/DR_DEU_Merging_MT_v01_shifted.xodr"]
        self._track_file_name = params_temp["TrackFilename",
                                            "Path to track file (csv)",
                                            os.path.expanduser('~') + "/STRIVE/data/strive/vehicle_tracks_005_adv.json"]
        ...
        self._strive_ds_reader = StriveDatasetReader()
    ...
    def __create_single_scenario__(self, scenario_track_info):
        ...
        for track_id in all_track_ids:
            ...
            agent = self._strive_ds_reader.AgentFromTrackfile(
