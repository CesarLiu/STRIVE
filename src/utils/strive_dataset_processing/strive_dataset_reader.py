...
from utils.interaction_dataset.python.utils.dict_utils import *
from utils.strive_dataset_processing.strive_scenario_decomposer import read_tracks
import numpy as np
import json
...
def TrajectoryFromTrack(track, xy_offset, start=0, end=None):
    states = list(get_item_iterator(track.motion_states))
    if end is None:
        end = states[-1][0]
...
class StriveDatasetReader: # adapted from InteractionDatasetReader
    """This class provides functions for extracting tracks and agents from StriveDataset scenarios."""
    ...
    def TrackFromTrackfile(self, filename, track_id, start_time, end_time):
        """Returns track of specific agent by reading trackfile."""
        if filename not in self._track_dict_cache:
            try:
                self._track_dict_cache[filename] = read_tracks(filename, start_time, end_time)
    ...
    def AgentFromTrackfile(self, track_params, param_server, scenario_track_info, agent_id, goal_def):
        """Returns BARK agent based on track information and agent ID."""
        ...
        fname = agent_track_info.GetFileName()
        track_id = agent_track_info.GetTrackId()
        agent_id = agent_track_info.GetTrackId()
        # start_time = scenario_track_info.GetStartTime()
        # end_time = scenario_track_info.GetEndTime()

        # track = self.TrackFromTrackfile(fname, track_id, start_time, end_time)
